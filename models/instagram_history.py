...
class InstagramHistory(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if res.provider_id and res.partner_id and res.partner_id.instagram_account_id and res.type != 'received':
                if res.message:
                    answer = False
                        ...
                        if res.provider_id.provider == "graph_api":
                            ...
                            if attachment_id.mimetype in image_type:
                                ...
                                media_url = {
                                    "link": base_url
                                    + "/web/content/"
                                    + str(res.sudo().attachment_ids.ids[0]),
                                    "filename": self.env["ir.attachment"].with_user(res.provider_id.user_id.id)
                                    .sudo()
                                ...
                                media_url = (
                                    base_url
                                    + "/web/content/"
                                    + str(res.sudo().attachment_ids[0].id)
                                )
